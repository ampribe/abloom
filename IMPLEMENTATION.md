...
$$i=(h>>32) \mathbin{\&} (\text{block\_count} - 1)$$

### 1.3 Bit Position Generation
The Parquet implementation defines 8 precomputed salts $s_i$ that can be combined with the lower 32 bits $\text{h\_low}$ of the hash to generate 8 randomly distributed indices. Here, the indices are in the range 0-63.
$$\text{bit}_i=(\text{h\_low}*s_i)>>26$$
