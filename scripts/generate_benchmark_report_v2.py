...
Usage:
    ...
    python scripts/generate_benchmark_report_v2.py results.json --baseline rbloom
    python scripts/generate_benchmark_report_v2.py results.json --output BENCHMARK.md
"""
...
def generate_report(
    ...
    lines.append("python scripts/generate_benchmark_report_v2.py results.json")
    lines.append("```")
    lines.append("")
    ...
    lines.append("```bash")
    for lib in libraries:
        lines.append(f'uv run pytest tests/test_benchmark.py -k "{lib}" --benchmark-only     # Only {lib}')
    lines.append("```")
    lines.append("")
